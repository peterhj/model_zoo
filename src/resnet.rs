...
pub fn residual_conv2d_op_gpu<Op>(x_dim: (usize, usize, usize), axes: Axes<(usize, usize)>, stats_cfg: BatchStatsConfig, stats_ctrl: &mut BatchStatsControl, param_vars: &mut VarSet, const_vars: &mut VarSet, x_: Rc<Op>) -> Rc<impl AVar<AData<DeviceBatchArray3d<f32>>>> where Op: 'static + AVar<AData<DeviceBatchArray3d<f32>>> {
...
pub fn residual_conv2d_prelu_op_gpu<Op>(x_dim: (usize, usize, usize), axes: Axes<(usize, usize)>, stats_cfg: BatchStatsConfig, stats_ctrl: &mut BatchStatsControl, param_vars: &mut VarSet, const_vars: &mut VarSet, x_: Rc<Op>) -> Rc<impl AVar<AData<DeviceBatchArray3d<f32>>>> where Op: 'static + AVar<AData<DeviceBatchArray3d<f32>>> {
  let shape1 = ConvShape{
    axes:       axes,
    kernel:     (3, 3),
    stride:     (1, 1),
  ...
  let y_dim = shape1.conv2d_output_dim(x_dim);
  assert_eq!(y_dim, shape2.conv2d_output_dim(y_dim));
  let y_ = batch_norm_conv2d_xavier_op_gpu(x_dim, shape1, stats_cfg, stats_ctrl, param_vars, const_vars, x_.clone());
  let y_ = symm_unit_clip_3d_op_gpu(param_vars, y_);
  let y_ = batch_norm_conv2d_xavier_op_gpu(x_dim, shape2, stats_cfg, stats_ctrl, param_vars, const_vars, y_);
  let y_ = x_.add(y_);
  y_
}

pub fn pad_residual_conv2d_op_gpu<Op>(x_dim: (usize, usize, usize), axes: Axes<(usize, usize)>, stride: (usize, usize), filters: usize, stats_cfg: BatchStatsConfig, stats_ctrl: &mut BatchStatsControl, param_vars: &mut VarSet, const_vars: &mut VarSet, x_: Rc<Op>) -> Rc<impl AVar<AData<DeviceBatchArray3d<f32>>>> where Op: 'static + AVar<AData<DeviceBatchArray3d<f32>>> {
  let shape1 = ConvShape{
    axes:       axes,
    kernel:     (3, 3),
    stride:     stride,
...
pub fn proj_residual_conv2d_prelu_op_gpu<Op>(x_dim: (usize, usize, usize), axes: Axes<(usize, usize)>, stride: (usize, usize), filters: usize, stats_cfg: BatchStatsConfig, stats_ctrl: &mut BatchStatsControl, param_vars: &mut VarSet, const_vars: &mut VarSet, x_: Rc<Op>) -> Rc<impl AVar<AData<DeviceBatchArray3d<f32>>>> where Op: 'static + AVar<AData<DeviceBatchArray3d<f32>>> {
  let proj_shape = ConvShape{
    axes:       axes,
    kernel:     (1, 1),
    stride:     stride,
  ...
  let y_ = batch_norm_conv2d_xavier_op_gpu(x_dim, shape1, stats_cfg, stats_ctrl, param_vars, const_vars, x_.clone());
  let y_ = symm_unit_clip_3d_op_gpu(param_vars, y_);
  let y_ = batch_norm_conv2d_xavier_op_gpu(y_dim, shape2, stats_cfg, stats_ctrl, param_vars, const_vars, y_);
  let y_ = proj_x_.add(y_);
  y_
}
...
pub fn imagenet_resnet18_prelu_model_gpu(batch_sz: usize) -> (CategoricalNLLLoss<DeviceBatchIoMem<u8>, DeviceIoBatch<u32>, DeviceBatchArray1d<f32>, DeviceIoBatch<f32>>, BatchStatsControl) {
  let mut input_vars = var_set();
  let mut label_vars = var_set();
  let mut prob_vars = var_set();
  let mut loss_vars = var_set();
  ...
  let y_ = batch_norm_conv2d_xavier_op_gpu(frame_dim, ConvShape{
    ...
    zero_pad:   true,
    filters:    Some(64),
  }, stats_cfg, &mut stats_ctrl, &mut param_vars, &mut const_vars, y_);
  let y_ = symm_unit_clip_3d_op_gpu(&mut param_vars, y_);
  ...
  let conv1_dim = (56, 56, 64);
  let y_ = residual_conv2d_prelu_op_gpu(conv1_dim, conv_axes, stats_cfg, &mut stats_ctrl, &mut param_vars, &mut const_vars, y_);
  let y_ = symm_unit_clip_3d_op_gpu(&mut param_vars, y_);
  let y_ = residual_conv2d_prelu_op_gpu(conv1_dim, conv_axes, stats_cfg, &mut stats_ctrl, &mut param_vars, &mut const_vars, y_);
  let y_ = symm_unit_clip_3d_op_gpu(&mut param_vars, y_);
  ...
  let y_ = proj_residual_conv2d_prelu_op_gpu(conv1_dim, conv_axes, (2, 2), 128, stats_cfg, &mut stats_ctrl, &mut param_vars, &mut const_vars, y_);
  let y_ = symm_unit_clip_3d_op_gpu(&mut param_vars, y_);
  let y_ = residual_conv2d_prelu_op_gpu(conv2_dim, conv_axes, stats_cfg, &mut stats_ctrl, &mut param_vars, &mut const_vars, y_);
  let y_ = symm_unit_clip_3d_op_gpu(&mut param_vars, y_);
  ...
  let y_ = proj_residual_conv2d_prelu_op_gpu(conv2_dim, conv_axes, (2, 2), 256, stats_cfg, &mut stats_ctrl, &mut param_vars, &mut const_vars, y_);
  let y_ = symm_unit_clip_3d_op_gpu(&mut param_vars, y_);
  let y_ = residual_conv2d_prelu_op_gpu(conv3_dim, conv_axes, stats_cfg, &mut stats_ctrl, &mut param_vars, &mut const_vars, y_);
  let y_ = symm_unit_clip_3d_op_gpu(&mut param_vars, y_);
  ...
  let y_ = proj_residual_conv2d_prelu_op_gpu(conv3_dim, conv_axes, (2, 2), 512, stats_cfg, &mut stats_ctrl, &mut param_vars, &mut const_vars, y_);
  let y_ = symm_unit_clip_3d_op_gpu(&mut param_vars, y_);
  let y_ = residual_conv2d_prelu_op_gpu(conv4_dim, conv_axes, stats_cfg, &mut stats_ctrl, &mut param_vars, &mut const_vars, y_);
  let y_ = symm_unit_clip_3d_op_gpu(&mut param_vars, y_);
